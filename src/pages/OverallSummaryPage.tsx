import { OverallSummaryData, Month, Quarter } from '../types/dashboard';
...
import { collapseOverallData } from '../utils/collapseData';

interface Props {
  data: OverallSummaryData;
}

export function OverallSummaryPage({ data }: Props) {
  const { collapsed } = useCollapse();
  const display = collapsed ? collapseOverallData(data) : data;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between px-6 pt-2 pb-1">
        ...
        >
          Overall Summary, {data.quarter}-{data.year}
        </h2>
        <SaptcoLogo />
      </div>
      ...
      <div className="flex flex-1 gap-2 px-3 pb-1 overflow-hidden" style={{ minHeight: 0 }}>
        ...
        {/* LEFT - Monthly trend + trendlines */}
        <div className="flex flex-col gap-1.5 flex-shrink-0" style={{ width: '27%', minWidth: 0 }}>
